...
* [Conv] Enabled grouped Composable Kernel (CK) xdlops convolution (2D and 3D; forward, backward-data, and weight) for tensors whose element strides exceed the int32 range, dispatching to CK's Large_Tensor int64-indexed instances. Backward-data and weight support uses the CK grouped-conv large-tensor instances from ROCm/rocm-libraries PR #9258.
